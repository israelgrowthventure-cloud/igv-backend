...
async def list_tasks(
    ...
    db = get_db()  # FIX: get_db() is synchronous — no await
    if db is None:
...
async def create_task(
    ...
    # FIX: log_audit_event(user, action, entity_type, entity_id, details)
    await log_audit_event(
        user=user,
        action="task_created",
        entity_type="task",
        entity_id=task_doc["id"],
        details={"title": task_doc["title"]}
    )
...
async def update_task(
    ...
    # FIX: correct log_audit_event signature
    await log_audit_event(
        user=user,
        action="task_updated",
        entity_type="task",
        entity_id=task_id,
        details={"changes": list(update_doc.keys())}
    )
